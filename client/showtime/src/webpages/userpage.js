...
export default class User extends React.Component {
  ...
  componentDidMount() {
    ...
    var configtickets = {
      method: "get",
      url: "http://localhost:3000/tickets/user/" + id,
      headers: {},
    };
    axios(configtickets)
      .then((res) => {
        ...
        <div class="flex flex-row mx-96 gap-96 mt-12">
          <div>
            ...
            <div class="border-2 border-red-700 rounded-md px-20 py-4 flex flex-col gap-2 w-96">
              ...
              <div> Nom : {this.state.users.lastname} </div>
              <div> Prénom : {this.state.users.firstname} </div>
              <div> Email : {this.state.users.email} </div>
              <div> Ville : {this.state.users.city} </div>
            </div>
            <h4>Mes tickets</h4>
            <div class="ml-32 border-2 rounded-md border-red-700 mt-8 w-80">
              <h1 class="font-semibold">Mes tickets</h1>
              {this.state.tickets.map((ticket) => {
                return (
                  <div key={ticket._id} class="flex">
                    <div>{ticket.event.event_name}</div>
                    <div>{ticket.event.price}</div>
                    <div>{ticket.event.location.location_name}</div>
                  </div>
                );
              })}
            </div>
