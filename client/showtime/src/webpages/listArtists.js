        ...
        <div class ="ml-72 mt-20 space-x-2 flex flex-row w-9/12 flex-wrap">
            {
                this.state.artists.map((artists) => {
                    return (
                        <div class ="font-semibold text-red-700" key={artists._id}> <a href={`/page_artist/${artists._id}`}>{artists.artist_name}
                        <img class="h-32" src={artists.photo} alt={artists.artist_name}/></a></div>
                    );
            })}
        </div>
    </div>);
