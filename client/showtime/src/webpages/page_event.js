...
export default function PageEvent() {
  ...
  React.useEffect(() => {
    axios.get("http://localhost:3000/events/" + _id).then((response) => {
      setPost(response.data);
      console.log(response.data);
    });
  }, []);

  function createticket(events) {
  ...
  if (!events) return "Cet évènement n'existe pas !";
  ...
  return (
    ...
    <div class="flex flex-col gap-7 mx-64 pt-10">
      <div class="flex flex-row gap-5 mx-96">
          ...
          <div>
            <img src={events.photo} class="h-96" alt="photo_event" />
          </div>
          <div class="flex flex-col gap-52">
            <div class="flex flex-col gap-1">
              <h1 class="text-lg font-semibold">{events.event_name} </h1>
              <h1> Lieu : {events.location.location_name}</h1>
              <p>Date : {events.event_date}</p>
              <p>Prix : {events.price}</p>
            </div>
        ...
        <div class="flex flex-col gap-2 mx-96">
          <h1 class="text-lg font-semibold">A propos de l'évènement</h1>
